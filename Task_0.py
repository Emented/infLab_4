...
def write_xml(list):
    f = open('Schedule_0.xml', 'w', encoding='UTF-8')
    f.write('\n'.join(list))
    f.close()
...
def find_tag(index):
    firstQuotes = start_json[index].find('"') + 1
    secondQuotes = start_json[index].find('"', firstQuotes)
    return start_json[index][firstQuotes:secondQuotes]


# функция нахождения текста
def find_text(index):
    textStart = start_json[i].find(tag) + len(tag) + 4
    textEnd = len(start_json[i]) - 2
    if start_json[i][-1] == ',':
        return start_json[index][textStart: textEnd]
    else:
        return start_json[index][textStart: textEnd + 1]
